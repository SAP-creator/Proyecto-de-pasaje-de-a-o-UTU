...
$path = str_replace('/Login/api/users/root', '', $path);
...
function opciones_http(string $metodo, string $ruta, ?array $datos){ 
    if (! (is_array($datos) && array_key_exists("token", $datos)) ){
        $res = new respuestaHTTP(400,json_encode(["error" =>"no se puede verificar si es root si no se envia un token de usuario"]));
        $res->enviar();
        return;
    }
    
    if (comprobar_token_tipo_usuario(["token"=>$datos["token"]],"root") != true){
        $res = new respuestaHTTP(400,json_encode(["error" =>"no se pudo verificar si el usuario es root o no."]));
        $res->enviar();
        return;
    }
    ...
    switch ($metodo) {
        ...
        default:
            ...
            $res =new respuestaHTTP(404,json_encode(["error"=> "no existe el metodo {$metodo} en root"]));
    }
    $res->enviar();
}
...
function opciones_POST(string $ruta,array $datos):respuestaHTTP{
    ...
    switch ($ruta){
        ...
        default:
            return new respuestaHTTP(404, json_encode(["error"=>"no existe la ruta {$ruta} en el metodo Post de root"]));
    }
...
function opciones_GET(string $ruta, array $datos):respuestaHTTP{
    ...
    switch ($ruta){
        ...
        case "/users/peticionesdata":

            $usuarios = todas_las_peticiones_usuario();
        ...
        default:
            return new respuestaHTTP(
                404,
                json_encode(["error"=>"no existe la ruta {$ruta} en el metodo GET de root"])
            );
    }
